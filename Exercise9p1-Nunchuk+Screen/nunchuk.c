...
static void parse_raw_nunchuk_data(uint8_t *raw_data)
{
    //the easy stuff
    joystick_x = raw_data[0];
    joystick_y = raw_data[1];
    ...
    */
    accel_x_raw = (raw_data[2]<<2) | ((raw_data[5]&0x0C)>>2);
    accel_y_raw = (raw_data[3]<<2) | ((raw_data[5]&0x30)>>4);
    accel_z_raw = (raw_data[4]<<2) | ((raw_data[5]&0xC0)>>6);
    ...
    c_button = (((raw_data[5]&0x02)>>1) ? true:false); 
    z_button = ((raw_data[5]&0x01) ? true:false);
}
...
static void convert_raw_accel_to_millig(accel_x_raw,accel_y_raw,accel_z_raw)
{
    accel_x_millig = map(accel_x_raw, ACCEL_LOWER_BOUND, ACCEL_UPPER_BOUND, 
    MILLIG_LOWER_BOUND, MILLIG_UPPER_BOUND);

    accel_y_millig = map(accel_y_raw, ACCEL_LOWER_BOUND, ACCEL_UPPER_BOUND, 
    MILLIG_LOWER_BOUND, MILLIG_UPPER_BOUND);

    accel_z_millig = map(accel_z_raw, ACCEL_LOWER_BOUND, ACCEL_UPPER_BOUND, 
    MILLIG_LOWER_BOUND, MILLIG_UPPER_BOUND);
}

void report_nunchuk_data(I2C_TypeDef * I2C, uint8_t I2C_address)
{
    *raw_data = read_raw_nunchuk_data(I2C, I2C_address);
    parse_raw_nunchuk_data(&raw_data);
